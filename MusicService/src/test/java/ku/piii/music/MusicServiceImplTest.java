...
public class MusicServiceImplTest {
    ...
    public void canDetectMissingArtistInJsonFile() {
        ...
        final MusicMediaCollection collectionWithArtist
                = instance.loadMusicMediaCollection(Paths.get(jsonFileWithArtist));
        final MusicMediaCollection collectionWithNoArtist
                = instance.loadMusicMediaCollection(Paths.get(jsonFileWithNoArtist));

        boolean comparisonResult = listsAreTheSame(collectionWithArtist.getMusic(),
                collectionWithNoArtist.getMusic());
        assertEquals(comparisonResult, false);
    ...
    public void canDetectMissingAlbumInJsonFile() {
        final String jsonFileWithArtist = "../test-json-files/Collection-B.json";
        final String jsonFileWithNoArtist = "../test-json-files/Collection-B-no-album.json";
        MusicService instance
                = new MusicServiceImpl(
                        new MusicRepositoryImpl(
                                new JacksonJSONMarshallingSupport(new ObjectMapper()),
    ...
    private boolean itemsAreTheSame(MusicMedia item1, MusicMedia item2) {
        ...
        if (stringsAreTheSame(item1.getAlbum(), item2.getAlbum()) ==  false){
            return false;
        }
        
        if (stringsAreTheSame(item1.getArtist(), item2.getArtist()) == false){
            return false;
        }
